...
function ActiveEffect(skill, name, lvl1MinDmg, lvl1MaxDmg, baseAS, procRate, damageType, proximity, extraTags) {
	...
	this.baseDmg = function() {
		...
		return avgDmg * (1+(0.1*level)) / (1 + 
			(this.isPhysical() ? skill.hero.getPhysicalBonusDmg(false) : 0) +
			(this.isEnergy() || this.isMental() ? skill.hero.getMentalBonusDmg(false) : 0) + 
			(skill.hero.baseStats.dmgRat ? (skill.hero.baseStats.dmgRat/enemyLvl*0.015) : 0) +
			(this.isMelee() && skill.hero.baseStats.dmgRat_melee ? (skill.hero.baseStats.dmgRat_melee/enemyLvl*0.015) : 0) +
			(this.isSummon() ? skill.hero.getSummonBonusDmg(false) : 0));
			
	};
	this.AS = function() {
		return this.isDot() || this.isMvt() || this.hasCooldown() || this.isSummon() ? this.baseAS : (this.baseAS * (1+skill.hero.getTotalStat("AS", skill)/100) * (this.isBarrage() ? (1+(skill.lvl-1)/100) : 1));
	};
	this.dmgRat = function() {
		return skill.hero.getTotalStat("dmgRat", skill) + 
			(this.isPhysical() ? skill.hero.getTotalStat("dmgRat_physical", skill) : 0) +
			(this.isEnergy() ? skill.hero.getTotalStat("dmgRat_energy", skill) : 0) +
			(this.isMental() ? skill.hero.getTotalStat("dmgRat_mental", skill) : 0) +
			(this.isMelee() ? skill.hero.getTotalStat("dmgRat_melee", skill) : 0) +
			(this.isRanged() ? skill.hero.getTotalStat("dmgRat_ranged", skill) : 0) +
			(this.isArea() ? skill.hero.getTotalStat("dmgRat_area", skill) : 0) +
			(this.isDot() ? skill.hero.getTotalStat("dmgRat_dot", skill) : 0);
	};
	this.dmgRat_Fact = function() { 
		return this.dmgRat()/enemyLvl*0.015 +
			skill.hero.getTotalStat("dmgRat_pc", skill)/100 +
			(this.isPhysical() ? skill.hero.getTotalStat("dmgRat_physical_pc", skill)/100 : 0) +
			(this.isEnergy() ? skill.hero.getTotalStat("dmgRat_energy_pc", skill)/100 : 0) +
			(this.isMental() ? skill.hero.getTotalStat("dmgRat_mental_pc", skill)/100 : 0) +
			(this.isMelee() ? skill.hero.getTotalStat("dmgRat_melee_pc", skill)/100 : 0) +
			(this.isRanged() ? skill.hero.getTotalStat("dmgRat_ranged_pc", skill)/100 : 0) +
			(this.isArea() ? skill.hero.getTotalStat("dmgRat_area_pc", skill)/100 : 0) +
			(this.isDot() ? skill.hero.getTotalStat("dmgRat_dot_pc", skill)/100 : 0) +
			(this.isSummon() ? skill.hero.getTotalStat("dmgRat_summon_pc", skill)/100 : 0);
	};
	this.dmg_Fact = function() {
		return this.dmgRat_Fact() +
			(this.isPhysical() ? skill.hero.getPhysicalBonusDmg(true) : 0) +
			(this.isEnergy() || this.isMental() ? skill.hero.getMentalBonusDmg(true) : 0) +
			(this.isSummon() ? skill.hero.getSummonBonusDmg(true) : 0);
	};
	this.dmg = function() {
		return this.baseDmg()*(1+this.dmg_Fact())
	};
	this.critRat_Fact = function() {
		var critRat = skill.hero.getTotalStat("critRat", skill) +
			(this.isPhysical() ? skill.hero.getTotalStat("critRat_physical", skill) : 0) +
			(this.isEnergy() ? skill.hero.getTotalStat("critRat_energy", skill) : 0) +
		    (this.isMental() ? skill.hero.getTotalStat("critRat_mental", skill) : 0) +
			(this.isMelee() ? skill.hero.getTotalStat("critRat_melee", skill) : 0) +
			(this.isRanged() ? skill.hero.getTotalStat("critRat_ranged", skill) : 0) +
			(this.isArea() ? skill.hero.getTotalStat("critRat_area", skill) : 0);
			
		var critRat_Fact = ((99 * critRat)/(critRat + 60 * enemyLvl + 1)/100);
		critRat_Fact += skill.hero.getTotalStat("critRat_pc", skill)/100;
		critRat_Fact += (this.isPhysical() ? skill.hero.getTotalStat("critRat_physical_pc", skill)/100 : 0);
		critRat_Fact += (this.isEnergy() ? skill.hero.getTotalStat("critRat_energy_pc", skill)/100 : 0);
		critRat_Fact += (this.isMental() ? skill.hero.getTotalStat("critRat_mental_pc", skill)/100 : 0);
		
		return critRat_Fact;
	};
	this.critDmg_Fact = function() {
		var critDmg = skill.hero.getTotalStat("critDmg", skill);
		var critDmg_Fact = ((150 + critDmg/enemyLvl * 0.75)/100);
		critDmg_Fact += skill.hero.getTotalStat("critDmg_pc", skill)/100;
		
		return critDmg_Fact;
	};
	this.brutRat_Fact = function() {
		var brutRat = skill.hero.getTotalStat("brutRat", skill);
		var brutRat_Fact = ((75 * brutRat)/(brutRat + 60 * enemyLvl + 1)/100);
		brutRat_Fact += skill.hero.getTotalStat("brutRat_pc", skill)/100;
		
		return brutRat_Fact;
	};
	this.brutDmg_Fact = function() {
		var critDmg = skill.hero.getTotalStat("critDmg", skill);
		var brutDmg = skill.hero.getTotalStat("brutDmg", skill);
		var brutDmg_Fact = ((300 + ((critDmg+brutDmg)/enemyLvl * 0.75))/100);
		brutDmg_Fact += skill.hero.getTotalStat("brutDmg_pc", skill)/100;
	...
	this.dps = function() {
		return this.dmg()*this.crit_Fact()*this.AS() + skill.hero.getProcs()*this.AS()*this.procRate;
	};
};
		...
		if (includeSkillScope || !this.isSkillScope()) {
			if (this.stat.name == statName && (!this.isBuff() || this.isBuffEnabled())) {
				if (this.name == "ARE") { // Arc Reactor Enhancement for Iron Man
					totalSpirit = skill.hero.getTotalStat("spirit", skill) + 60;
					total += totalSpirit * 3;
				}
			}
		}
